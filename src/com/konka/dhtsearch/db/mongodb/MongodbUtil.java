...
public class MongodbUtil {
	...
	public void save(Object object) throws Exception {
		...
		// System.out.println("鏄泦鍚堝悧=" + dbObject2);
		// }
		// }
	}
	...
	public static String getDBCollectionName(Class<?> clazz) {
		...
		if (collectionAnno != null && collectionAnno.value().trim().length() != 0) {
			...
			collectionName = clazz.getName().replace(".", "_");// mongodb涓嶆敮鎸�'.'鐨勫悕绉�
		}
	...
	// $set 娉ㄦ剰 DBObject updateSetValue = new BasicDBObject("$set", updatedValue);
	// 鍔犱笂set 鑷充慨鏀规坊鍔犵殑閮ㄥ垎锛屽叾浠栨暟鎹笉鍙橈紝
	/**
	 * 璺熸柊
	 * 
	 * @param clazz
	 ...
	 *            璁剧疆鏂板��
	 * @param isOverWrite
	 *            ture 瑕嗙洊鍏ㄩ儴鍊硷紝 false 鍊间慨鏀硅瀹氱殑閮ㄥ垎锛屼箣鍓嶇殑涓嶅彉
	 */
	...
	public <T> List<T> find(Class<T> clazz, BasicDBObject where, int limit) throws Exception {
		...
		System.out.println(curr.count() + "锛氶」");
		while (curr.hasNext()) {
			DBObject dbo = curr.next();
	...
	public <T> T loadOne(Class<T> clazz, DBObject dbo) throws Exception {
		...
		for (Field f : fields) {
			...
			if (value != null) {
				...
				value = valueDeserial(type, value);// 瑙ｆ瀽寰楀埌鐪熷疄鍊�
			}
	...
	private Object valueDeserial(Type type, Object object) throws Exception {
		...
		if (checkValidType(clazz)) {// 濡傛灉鏄熀鏈被鍨嬶紝鐩存帴杩斿洖
			return object;
		}
